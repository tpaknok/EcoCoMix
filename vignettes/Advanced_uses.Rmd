...
## [1] "LAI"
```
This, however, will use default priors for everything. Have a look at the prior specification. Note that INLA uses precision instead of variance.
...
Apart from the fixed effects, which uses a gaussian distribution, all other parameters are estimated using logGamma distribution. This is because the internal calculations are conducted at log scale before back-transforming them to the original scale. Therefore, the priors must be specificed at log scale too. You can use \code{inla.doc("")} to check the internal calculations of different distributions and priors. For gaussian distribution, use \code{inla.doc("Gaussian")}.

Note that if the log-transformed hyperparameters follow logGamma(a,b), the original hyperparameters follow Gamma(a,b). Read \code{inla.doc("Gamma")} to learn more!
...
![plot of chunk prior investigation](figure/prior investigation-1.png)

```r
plot(density(rgamma(1000,shape=1,rate=5e-5)))
```

![plot of chunk prior investigation](figure/prior investigation-2.png)
...
## [1] "LAI"
```
Yo only need to specify the manual prior in \code{priors}. For example, if you want to use the default prior for block, just use \code{list(prior.phy=prior}. Note that the name of the priors shouldn't contain the word "Phylo" - it is designated to hold the communtiy phylogenetic covariance matrix. If you name the prior with the word "Phylo", an error would occur.
