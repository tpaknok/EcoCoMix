...
m <- gls(log_flwr_total~Real.rich,data=KSR_EF)
...
ML.opt2<-optim(runif(1,0.2,0.8),
                 ...
                 method="L-BFGS-B",
                 lower=0.0,upper=1.0,control=list(factr=1e9)) #SLOW!
...
For the optimized lambda model, the 95%CI of the estimates of richness is In my case, I used lapply to do it. So the coding is more complicated.
...
ef_empiricial <- function(ef_data,x,VCV_sp,names,ef_properties,comm) {
  print(ef_properties)
  y <- ef_data
  ...
  if (ef_properties == "bug.rich" | ef_properties == "bugs" | ef_properties == "poll_total") {
    family <- "poisson"
  } else {
    family <- "gaussian"
  }
